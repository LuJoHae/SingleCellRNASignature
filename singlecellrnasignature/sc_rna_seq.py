    ...
    def derive(self, lair: Lair) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for prefix in prefixes:
                data = mmread(extract_dir.joinpath(prefix+"_matrix.mtx.gz")).T
...
class BiTumorImmuneReprogramming2021Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        dir_path = Path(filepaths["SCP1288"].joinpath("expression", "60c76a18771a5b0ba10ea91b"))
        data = mmread(dir_path.joinpath("matrix.mtx")).T.tocsr()
...
class BiermannDissectingTreatmentnaiveEcosystem2022Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        data = mmread(filepaths["GSE200218_sc_sn_counts.mtx.gz"]).T.tocsr()
...
class BorcherdingMappingImmuneEnvironment2021Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            adatas = []
            for prefix in prefixes:
                data = mmread(extract_dir.joinpath("_".join([prefix, "matrix.mtx.gz"]))).T.tocsr()
...
class ChengPancancerSinglecellTranscriptional2021Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        filepaths = lair.get_dataset_filepaths(raw_dataset)
        ...
        for i, cancer_type in enumerate(cancer_types):
            ...
            adata = ad.AnnData(csr_matrix(np.array(df.values, dtype=np.float64)), obs=metadata)
...
class KhaliqRefiningColorectalCancer2022Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        data = csr_matrix(df.select(df.columns[1:]).to_numpy()).T
    ...
    def derive(self, lair: Lair) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for sub_dir in extract_dir.iterdir():
                ...
                data = mmread(list(filter(lambda x: "matrix" in x.name, files))[0]).tocsr().T
                adata = ad.AnnData(data)
                adata.obs = barcodes
...
class LuSinglecellAtlasMulticellular2022Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            data = csr_matrix(data.drop("gene").to_numpy()).T
...
class SharmaOncofetalReprogrammingEndothelial2020Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        adata = ad.AnnData(mmread(filepaths["GSE156625_HCCFmatrix.mtx.gz"]).T.tocsr())
        ...
        bdata = ad.AnnData(mmread(filepaths["GSE156625_HCCmatrix.mtx.gz"]).T.tocsr())
...
class ZhangSinglecellAnalysesReveal2021Adata(DatasetscRNASeqSignature):
    ...
    def derive(self, lair: Lair) -> None:
        ...
        adata = ad.AnnData(mmread(filepaths["GSE169246_TNBC_RNA.counts.mtx.gz"]).T.tocsr())
